...
if ($search !== '') {
    $like = "%$search%";
    $where = " WHERE (s.id LIKE ? OR s.username LIKE ? LIKE ?)";
    $types = "sss";
...
// Fetch students
$sql = "
    SELECT s.id, s.username, block_number,
           s.block_id
    FROM proctors s inner JOIN blocks b ON s.block_id = b.id
    $where
    ORDER BY s.id
";
                ...
                <?php while ($student = $students->fetch_assoc()): ?>
                    <div class="student-card">
                        ...
                        <div class="info-row">
                            ...
                            <span><?= $student['block_id'] ? "Block {$student['block_number']}" : 'Not assigned' ?></span>
                        </div>
                    </div>
                <?php endwhile; ?>
            </div>
